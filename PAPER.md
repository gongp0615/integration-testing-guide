# DSMB-Agent：基于命令树与步进思考的 Agent 驱动集成测试方法

> **DSMB-Agent: Agent-Driven Integration Testing via Tree-Structured Command Space and Step-Wise Reasoning**

---

## 摘要

大语言模型（LLM）大幅提升了代码生成效率，但系统级验证仍高度依赖人工编写测试用例。随着开发速度加快，模块间关联日益复杂，传统方法面临结构性瓶颈。本文提出 DSMB-Agent，一种面向复杂系统的 Agent 驱动集成测试方法，通过三阶段流程（理解—设计—执行）将大部分系统级验收工作从开发者转移到 Agent。该方法基于两个核心机制：（1）树状命令空间——将被测系统暴露为"模块→命令→参数表"的层级结构，Agent 既能使用已有命令，也能在受控边界内自主注册新命令以扩展测试能力；（2）步进思考——通过断点控制器将执行拆分为单步，使 Agent 在每步之后观察因果链并动态调整测试方向。在一个包含 6 个业务模块和 7 个预埋缺陷的事件驱动游戏服务端原型上，我们设计了三级自主性模式（L0/L1/L2）和 2×2 消融实验。实验结果表明：步进执行组的缺陷发现 F1 达 0.769，批量执行组仅 0.267；L1 模式下 Agent 通过注册新命令成功复现了所有 L2 组未能触及的边界缺陷 B1；L0 模式下 Agent 从零注册 7 条命令并发现 6 个缺陷。
...
3. 设计三级自主性分级（L0/L1/L2）和 2×2 消融实验，量化评估命令注册与步进执行各自的贡献（第 4-5 节）；
4. 在事件驱动系统原型上进行初步验证，步进执行组 F1 达 0.769（vs. 批量组 0.267），L1 模式通过命令注册突破了所有 L2 组的接口盲区（第 5 节）。
...
| 参数 | GLM-5.1 | GPT-5.4 |
|------|---------|---------|
| 提供商 | 智谱 AI（API） | OpenAI（Codex CLI） |
| 最大迭代次数 | 80 | 80 |
| L2 每组运行次数 | 1 | 3 |
| L0 运行次数 | 1 | 2 |
| L1 运行次数 | 1 | — |
...
### 5.1 L2 消融实验（GLM-5.1，单次运行）

表 1 报告了四个 L2 实验组及 code-only 基线在 GLM-5.1 上的表现：

**表 1：L2 消融实验结果（GLM-5.1，单次运行）**

| 指标 | D（双模式） | B（步进） | C（代码+批量） | A（仅批量） | code-only |
|------|:-----------:|:---------:|:--------------:|:-----------:|:---------:|
| 发现缺陷数 | 4 | 5 | 3 | 2 | 6 |
| 精确率 | 80% | 83% | 75% | 25% | 60% |
| 召回率 | 57% | 71% | 43% | 29% | 86% |
| F1 | 0.667 | 0.769 | 0.545 | 0.267 | 0.706 |
| 难度加权分 | 8.5/14 | 10.0/14 | 7.0/14 | 3.5/14 | 13.0/14 |
| 关联召回率 | 100% | 70% | 70% | 60% | 100% |
| 误报率 | 20% | 17% | 25% | 75% | 40% |
| 迭代次数 | 105 | 94 | 70 | 62 | 25 |

**消融因子分析：**

| 因子 | 关联召回率增益 | 缺陷召回率增益 | 难度加权分增益 |
|------|:-------------:|:-------------:|:-------------:|
| 代码理解主效应 | +0.20 | +0.00 | +0.07 |
| 步进执行主效应 | +0.20 | +0.29 | +0.29 |
| 交互效应 | +0.20 | −0.29 | −0.36 |
...
**表 3：L2 汇总指标（GPT-5.4，每组 3 次运行的均值）**
...
**发现 1：代码理解+运行时组合在 GPT-5.4 上表现最强。** 与 GLM-5.1 上 B 组（step-only）表现最优不同，GPT-5.4 上 C 组（code-batch，平均 5.0/7）和 D 组（dual，平均 4.0/7）均优于 B 组（step-only，平均 2.0/7）。这表明 GLM-5.1 上观察到的负交互效应可能与模型的代码理解能力有关，而非方法本身的固有缺陷。GPT-5.4 具备更强的代码理解能力后，代码+运行时的组合能充分发挥优势。

**发现 2：B1 仍然是所有 L2 组的盲区。** 在两个模型的 L2 实验中，B1（`RemoveItem` 负数参数）均未被任何运行时组发现。只有 code-only 通过静态分析识别了该缺陷。这进一步验证了命令注册机制的必要性。

**发现 3：步进执行在 GPT-5.4 上的优势减弱。** A 组（batch-only，1.7/7）与 B 组（step-only，2.0/7）差距缩小。分析原因：GPT-5.4 在 batch 模式下也能从累积日志中推断因果关系，部分弥补了缺少逐步观察的劣势。但 C 组（code-batch，5.0/7）的优异表现说明，代码理解对 batch 模式的增益远大于步进执行单独的增益。

**发现 4：code-only 的跨模型稳定性最高。** code-only 在两个模型上均达到约 4-6/7 的缺陷发现率，且是唯一能发现 B1 的组别。但其固有局限是无法通过运行时验证排除误报。

**发现 5：D 组（dual）在 GPT-5.4 上表现稳定。** 3 次运行均发现 B2、B3、B4、B6，展现了代码+步进组合的可靠性。

### 5.3 跨模型对比

将两个模型的结果合并观察：

| 组别 | GLM-5.1（1 次） | GPT-5.4（3 次均值） | 趋势 |
|------|:--------------:|:------------------:|------|
| A（仅批量） | 2 | 1.7 | 一致：最弱 |
| B（步进） | 5 | 2.0 | GPT-5.4 下降 |
| C（代码+批量） | 3 | 5.0 | GPT-5.4 显著提升 |
| D（双模式） | 4 | 4.0 | 一致 |
| code-only | 6 | 4.0 | GPT-5.4 下降 |

**核心洞察：** 代码理解的效果高度依赖模型能力。GLM-5.1 上代码理解的边际收益为零甚至为负（消融因子 +0.00），而 GPT-5.4 上代码理解显著提升了 batch 模式的表现（C 组从 3 提升到 5.0）。这意味着随着 LLM 代码理解能力的提升，"代码+运行时"的组合模式将比"纯运行时"模式更具优势。

### 5.4 L0 实验结果

L0 模式下 Agent 从零开始，没有任何预构建业务命令。

**GLM-5.1（1 次运行）：** Agent 自主注册了 7 条测试命令，遍历全部 6 个子系统，发现 6 个缺陷，正确识别了 `mail.claimed` 事件断链（B6）。
...
| 迭代次数 | 80 | 70 |
| 注册命令数 | 7 | 7 |
| 发现缺陷 | B1, B3, B4, B5, B6（5/7） | B1, B3, B4, B5（4/7） |

两次运行均成功注册了完整的测试命令集并发现了 B1——这是所有 L2 组无法触及的缺陷。GPT-5.4 的 L0 结果与 GLM-5.1 一致，验证了全自主模式的跨模型可行性。

### 5.5 L1 实验结果（GLM-5.1）

L1 模式的核心验证目标是"接口缺口"假说。Agent 额外注册了 2 条自定义命令（`test_remove_negative`、`test_add_zero`），首先通过内建命令传入 `count=-1` 被命令层拒绝，随后注册 `test_remove_negative` 直接调用底层方法，观察到物品数量从 1 变为 2——**成功复现了所有 L2 组均未能发现的 B1 缺陷**。

L1 运行总计发现 6 个缺陷，涵盖 B1、B3、B4、B5、B6 及装备槽位校验缺失。这是"理解—设计—执行"三阶段闭环的完整演示。
...
L2 全部四组均未能发现 B1，而 L1 通过命令注册成功复现了该缺陷。这一对比表明：当测试接口无法构造出触发缺陷所需的输入时，再强的探索策略也无法弥补接口本身的不足。命令注册机制使 Agent 从"命令使用者"升级为"命令设计者"，是本文方法区别于传统自动化测试的核心差异。

### 6.2 世界暂停赋予 Agent 主动干预和选择测试方向的能力

步进执行相比批量执行的优势，不仅在于粒度更细或因果可追溯，核心在于**世界暂停（Hold World）**为 Agent 创造了自主决策的空间。在每步执行后，世界完全静止，Agent 可以自由地查询状态、修改数据、注册新接口——主动构造测试条件而非被动等待结果。批量执行下 Agent 必须在执行前确定所有操作，世界持续运转，Agent 失去的不仅是观察粒度，更是在每步之间主动干预系统、构造前置条件和调整探索方向的能力。实验中 B 组（步进）发现 5/7 个缺陷的过程直接体现了这一点：Agent 在暂停期间决定重复操作验证幂等性（发现 B2）、查询关联模块追踪链式影响、注册新命令突破接口限制（发现 B1）——这些决策都是在世界暂停的窗口中动态生成的，而非预先规划。

### 6.3 代码理解的效果依赖模型能力

GLM-5.1 上 D 组表现低于 B 组，而 GPT-5.4 上 C 组（code-batch，5.0/7）显著优于 B 组（step-only，2.0/7）。这一跨模型对比揭示了一个重要洞察：代码理解的边际收益不是固定的，而是随模型的代码理解能力增长。GLM-5.1 上观察到的负交互效应部分源于模型在代码阅读上消耗了大量迭代却未能提取足够的有效信息。当模型代码理解能力更强（GPT-5.4）或工具更高效（LSP 语义导航替代文本搜索）时，代码理解阶段的投入回报率会显著提升。
...
code-only 基线的高召回率（86%）和高误报率（40%）表明：静态分析擅长生成"怀疑"，运行时测试擅长确认或排除怀疑。两者是互补关系，非替代关系。一个可能的改进方向是将静态分析作为运行时测试的优先级排序器——Agent 先通过代码分析标记可疑区域，再将有限的运行时预算集中在高优先级区域。
...
**运行次数不均衡：** GLM-5.1 实验为单次运行，GPT-5.4 的 L2 消融实验完成了每组 3 次运行，但 L0 仅完成 2 次，L1 仅有 GLM-5.1 的 1 次数据。GPT-5.4 的 3 次运行已能观察到一定的稳定性趋势（如 dual 组 3 次均发现 B2/B3/B4/B6），但仍不足以进行严格的统计检验。后续需要补充更多轮次并报告均值和标准差。

**预埋缺陷的代表性：** 7 个缺陷由作者人工设计和预埋，可能无法完全代表真实系统的缺陷分布。缺陷设计参考了常见的事件驱动系统缺陷模式，但在后续工作中应考虑引入 mutation testing 方法论来增加客观性。

**迭代预算的影响：** 最大迭代次数设为 80，不同组的实际迭代消耗不同（62-105），这可能影响对各组能力上限的公平比较。
...
1. 代码理解+运行时测试的组合是最有效的策略——GPT-5.4 上 C 组（code-batch）平均发现 5.0/7 个缺陷，D 组（dual）稳定在 4.0/7，均优于纯运行时组；
2. 代码理解的效果高度依赖模型能力——GLM-5.1 上代码理解的边际收益接近零，而 GPT-5.4 上代码理解使 batch 模式的表现从 1.7 提升到 5.0；
3. B1 是所有 L2 组在两个模型上的共同盲区——命令注册能力（L0/L1）是突破接口盲区的关键；
4. L0（全自主）模式在两个模型上均能从零构建完整测试命令集并发现 4-6 个缺陷，验证了方法的跨模型可行性；
5. 静态分析与运行时测试互为补充——code-only 是唯一能稳定发现 B1 的 L2 组别，但无法排除误报。

### 8.2 未来工作

- **多轮重复实验与统计检验**——在统一条件下完成更多轮次的正式对照比较并进行统计显著性检验；
...
[10] C. Lemieux et al., "CodaMosa: Escaping Coverage Plateaus in Test Generation with Pre-trained Large Language Models," in Proc. ICSE, 2023.

[11] Y. Deng et al., "Large Language Models Are Zero-Shot Fuzzers: Fuzzing Deep-Learning Libraries via Large Language Models," in Proc. ISSTA, 2023.
...
[16] L. Zhang et al., "An Empirical Study on Bug Characteristics in Event-Driven Programs," in Proc. ASE, 2019.
...
- L0 首轮结果：`results/l0-run1-report.md`
- L1 首轮结果：`results/l1-run1-report.md`
- L2 消融实验报告：`results/evaluation_report.md`
